...
// TestResourceFormatDraft tests that resource responses are formatted according to the draft spec
func TestResourceFormatDraft(t *testing.T) {
	// Create a server
	srv := server.NewServer("spec-test-server-draft")
	...
	t.Run("StringResource", func(t *testing.T) {
		...
		responseBytes, err := server.HandleMessageWithVersion(srv.GetServer(), requestJSON, "draft")
		if err != nil {
