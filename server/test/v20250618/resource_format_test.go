...
// TestResourceFormatV20250618 tests that resource responses are formatted according to the draft spec
func TestResourceFormatV20250618(t *testing.T) {
	// Create a server
	srv := server.NewServer("spec-test-server-v20250618")
	...
	t.Run("StringResource", func(t *testing.T) {
		...
		responseBytes, err := server.HandleMessageWithVersion(srv.GetServer(), requestJSON, "2025-06-18")
		if err != nil {
		...
		// According to the draft spec, the result should have a contents array
