...
var (
	...
	promptStore = make(map[string]gomcp.Prompt)
...
func handleEcho(ctx context.Context, progressToken *gomcp.ProgressToken, arguments map[string]interface{}) (content []gomcp.Content, isError bool) {
	...
	if !ok {
		errContent := gomcp.TextContent{Type: "text", Text: "Error: Invalid or missing 'message' argument (string expected)"}
		return []gomcp.Content{errContent}, true // Indicate error
	}
	respContent := gomcp.TextContent{Type: "text", Text: fmt.Sprintf("Echo: %s", message)}
	return []gomcp.Content{respContent}, false
}

func handleAdd(ctx context.Context, progressToken *gomcp.ProgressToken, arguments map[string]interface{}) (content []gomcp.Content, isError bool) {
	...
	if !ok1 || !ok2 {
		errContent := gomcp.TextContent{Type: "text", Text: "Error: Invalid or missing 'a' or 'b' arguments (number expected)"}
		return []gomcp.Content{errContent}, true // Indicate error
	}
	sum := a + b
	respContent := gomcp.TextContent{Type: "text", Text: fmt.Sprintf("The sum of %f and %f is %f.", a, b, sum)}
	return []gomcp.Content{respContent}, false
}

// handleLongRunning needs access to the server instance to send progress
func handleLongRunning(server *gomcp.Server) gomcp.ToolHandlerFunc {
	...
	return func(ctx context.Context, progressToken *gomcp.ProgressToken, arguments map[string]interface{}) (content []gomcp.Content, isError bool) {
		...
		for i := 1; i <= steps; i++ {
			...
			case <-ctx.Done(): // Check for cancellation
				...
				errContent := gomcp.TextContent{Type: "text", Text: fmt.Sprintf("Operation cancelled by client at step %d", i)}
				return []gomcp.Content{errContent}, true // Indicate error
			case <-time.After(stepDuration):
				...
				if progressToken != nil {
					...
					progParams := gomcp.ProgressParams{Token: string(*progressToken), Value: progressPayload}
		...
		respContent := gomcp.TextContent{Type: "text", Text: resultText}
		return []gomcp.Content{respContent}, false
	}
}

func handleGetTinyImage(ctx context.Context, progressToken *gomcp.ProgressToken, arguments map[string]interface{}) (content []gomcp.Content, isError bool) {
	...
	imgContent := gomcp.ImageContent{
		Type:      "image",
		Data:      MCP_TINY_IMAGE_BASE64, // Use the base64 constant
		MediaType: "image/png",
		Annotations: &gomcp.ContentAnnotations{
			Title: StringPtr("MCP Logo Tiny"), // Use helper for pointer
		},
	}
	textBefore := gomcp.TextContent{Type: "text", Text: "This is a tiny image:"}
	textAfter := gomcp.TextContent{Type: "text", Text: "The image above is the MCP tiny image."}

	return []gomcp.Content{textBefore, imgContent, textAfter}, false
}
...
func handleGetPromptCustom(s *gomcp.Server, requestID interface{}, params interface{}) error {
	log.Println("[Handler] Custom GetPrompt logic executing")
	var requestParams gomcp.GetPromptRequestParams
	if err := gomcp.UnmarshalPayload(params, &requestParams); err != nil {
		return s.Conn().SendErrorResponse(requestID, gomcp.ErrorPayload{
			Code: gomcp.ErrorCodeInvalidParams, Message: fmt.Sprintf("Failed to unmarshal GetPrompt params: %v", err),
		})
	}
	...
	if !exists {
		...
		return s.Conn().SendErrorResponse(requestID, gomcp.ErrorPayload{
			Code:    gomcp.ErrorCodeMCPResourceNotFound,
			Message: fmt.Sprintf("Prompt template not found: %s", requestParams.URI),
		})
	}

	// --- Basic Templating Logic ---
	renderedMessages := make([]gomcp.PromptMessage, len(promptTmpl.Messages))
	...
	if requestParams.Arguments != nil {
		for _, argDef := range promptTmpl.Arguments {
			if val, ok := requestParams.Arguments[argDef.Name]; ok {
				...
				return s.Conn().SendErrorResponse(requestID, gomcp.ErrorPayload{
					Code:    gomcp.ErrorCodeInvalidParams,
	...
	for i, msg := range promptTmpl.Messages { // Iterate over the original template messages
		renderedContent := make([]gomcp.Content, len(msg.Content)) // Create the slice for the new message's content
		for j, contentItem := range msg.Content {
			if textContent, ok := contentItem.(gomcp.TextContent); ok {
				...
				renderedContent[j] = gomcp.TextContent{
		...
		renderedMessages[i] = gomcp.PromptMessage{Role: msg.Role, Content: renderedContent}
	}
	...
	responsePayload := gomcp.GetPromptResult{Prompt: responsePrompt}
...
func handleReadResourceCustom(s *gomcp.Server, requestID interface{}, params interface{}) error {
	log.Println("[Handler] Custom ReadResource logic executing")
	var requestParams gomcp.ReadResourceRequestParams
	if err := gomcp.UnmarshalPayload(params, &requestParams); err != nil {
		return s.Conn().SendErrorResponse(requestID, gomcp.ErrorPayload{
			Code: gomcp.ErrorCodeInvalidParams, Message: fmt.Sprintf("Failed to unmarshal ReadResource params: %v", err),
		})
	}
	...
	if requestParams.Version != "" && requestParams.Version != currentVersion {
		...
		return s.Conn().SendErrorResponse(requestID, gomcp.ErrorPayload{
			Code:    gomcp.ErrorCodeInvalidParams, // Or a more specific version mismatch code
	...
	var resourceContents gomcp.ResourceContents
	...
	if strings.HasPrefix(contentType, "text/") || contentType == "application/json" || contentType == "application/xml" {
		resourceContents = gomcp.TextResourceContents{
		...
		resourceContents = gomcp.BlobResourceContents{
			ContentType: contentType,
			Blob:        content, // Use Blob field name
		}
	}

	responsePayload := gomcp.ReadResourceResult{
...
func main() {
	...
	server := gomcp.NewServer("GoMCPKitchenSinkServer")
	...
	longRunningTool := gomcp.Tool{
		...
		Annotations: gomcp.ToolAnnotations{ // Example annotation
	...
	getTinyImageTool := gomcp.Tool{
		Name:        ToolGetTinyImage,
		Description: "Returns a tiny base64 encoded PNG image.",
		InputSchema: gomcp.ToolInputSchema{Type: "object"}, // No input arguments
		Annotations: gomcp.ToolAnnotations{ReadOnlyHint: BoolPtr(true)},
	}
	...
	simplePrompt := gomcp.Prompt{
		URI:         PromptSimple,
		Title:       "Simple Prompt",
		Description: "A basic prompt with no arguments.",
		Messages: []gomcp.PromptMessage{
			{Role: "user", Content: []gomcp.Content{gomcp.TextContent{Type: "text", Text: "Tell me a short joke."}}},
		},
	}
	promptMu.Lock()
	...
	complexPrompt := gomcp.Prompt{
		...
		Messages: []gomcp.PromptMessage{
			{Role: "system", Content: []gomcp.Content{gomcp.TextContent{Type: "text", Text: "You are a helpful assistant."}}},
			{Role: "user", Content: []gomcp.Content{
				gomcp.TextContent{Type: "text", Text: "Please explain ${topic} in a ${style:-casual} style."},
				gomcp.ImageContent{Type: "image", Data: MCP_TINY_IMAGE_BASE64, MediaType: "image/png"}, // Use Data field
			}},
		},
	}
	promptMu.Lock()
	...
	// Example: server.RegisterNotificationHandler(gomcp.MethodProgress, handleClientNotification)
...
func simulateResourceUpdates(s *gomcp.Server) {
