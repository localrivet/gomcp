...
func executeCalculator(ctx context.Context, progressToken interface{}, args any) (content []protocol.Content, isError bool) { // Use protocol types
	...
	if !ok {
		return newErrorContent("Invalid arguments for calculator tool (expected object)"), true
	}

	op1Arg, ok1 := argsMap["operand1"]
	op2Arg, ok2 := argsMap["operand2"]
	opStrArg, ok3 := argsMap["operation"]
