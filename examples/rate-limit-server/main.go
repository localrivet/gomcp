...
func limitedEchoHandler(ctx context.Context, progressToken *protocol.ProgressToken, arguments map[string]interface{}) (content []protocol.Content, isError bool) {
	...
	if !globalLimiter.Allow() {
		...
		// Return a specific MCP error code for rate limiting
		// Note: The client example checks for this specific text message.
		// A better approach might be to define and use protocol.ErrorCodeMCPRateLimitExceeded
		// if the spec supports it or use a custom error code.
		return newErrorContent(fmt.Sprintf("Too many requests. Limit is %d per second (burst %d). ErrorCode: %d",
			requestsPerSecond, burstLimit, protocol.ErrorCodeMCPRateLimitExceeded)), true
	}
	...
	if !ok {
		return newErrorContent("Missing required argument 'message' for tool 'limited-echo'"), true
	}
	messageStr, ok := messageArg.(string)
	if !ok {
		return newErrorContent("Argument 'message' for tool 'limited-echo' must be a string"), true
	}
...
func main() {
	...
	err := srv.RegisterTool(limitedEchoTool, limitedEchoHandler)
	if err != nil {
