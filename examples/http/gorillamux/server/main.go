...
func main() {
	logger := &simpleLogger{}
	listenAddr := "127.0.0.1:8088" // Use a different port for the gorilla/mux example

	// 1. Create the core MCP Server
	coreServer := mcpServer.NewServer(
		"GorillaMuxExampleMCPServer", // Updated server name
	...
	// 3. Create a gorilla/mux router
	r := mux.NewRouter()

	// 4. Register the SSE transport handlers with the gorilla/mux router
	r.HandleFunc("/sse", sseTransport.HandleSSE).Methods(http.MethodGet)
	r.HandleFunc("/message", sseTransport.HandleMessage).Methods(http.MethodPost)

	// Add a simple root handler for testing the server itself
	r.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		w.Write([]byte("gorilla/mux server running. MCP endpoints at /sse and /message"))
	}).Methods(http.MethodGet)

	// 5. Create and configure the HTTP server using the gorilla/mux router
	...
	go func() {
		logger.Info("Starting MCP Server with gorilla/mux router on %s...", listenAddr)
